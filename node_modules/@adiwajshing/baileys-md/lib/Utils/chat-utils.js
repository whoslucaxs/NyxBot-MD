...
const computeLtHash = (initial, macs, getPrevSetValueMac) => {
    ...
    for (let i = 0; i < macs.length; i++) {
        ...
        if (subOp) {
            if (subOp.operation === WAProto_1.proto.SyncdMutation.SyncdMutationSyncdOperation.SET) {
...
const encodeSyncdPatch = async ({ type, index, syncAction, apiVersion }, { creds: { myAppStateKeyId }, keys }) => {
    ...
    state.hash = computeLtHash(state.hash, [{ indexMac, valueMac, operation }], (index) => [...state.mutations].reverse().find(m => Buffer.compare(m.indexMac, index) === 0));
    state.version += 1;
...
const decodeSyncdPatch = async (msg, name, getAppStateSyncKey, validateMacs = true) => {
    const keyCache = {};
...
const decodePatches = async (name, syncds, initial, getAppStateSyncKey, validateMacs = true) => {
    ...
    for (const syncd of syncds) {
        ...
        const decodeResult = await exports.decodeSyncdPatch(syncd, name, getAppStateSyncKey, validateMacs);
